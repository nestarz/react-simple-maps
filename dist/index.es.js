...
var versor = function(e) {
  var l = e[0] / 2 * radians$1, sl = sin$3(l), cl = cos$3(l), // λ / 2
      p = e[1] / 2 * radians$1, sp = sin$3(p), cp = cos$3(p), // φ / 2
      g = e[2] / 2 * radians$1, sg = sin$3(g), cg = cos$3(g); // γ / 2
  return [
    cl * cp * cg + sl * sp * sg,
    sl * cp * cg - cl * sp * sg,
    cl * sp * cg + sl * cp * sg,
...
versor.cartesian = function(e) {
  var l = e[0] * radians$1, p = e[1] * radians$1, cp = cos$3(p);
...
versor.rotation = function(q) {
  return [
    atan2$2(2 * (q[0] * q[1] + q[2] * q[3]), 1 - 2 * (q[1] * q[1] + q[2] * q[2])) * degrees$2,
    asin$2(max$3(-1, min$2(1, 2 * (q[0] * q[2] - q[3] * q[1])))) * degrees$2,
    atan2$2(2 * (q[0] * q[3] + q[1] * q[2]), 1 - 2 * (q[2] * q[2] + q[3] * q[3])) * degrees$2
  ];
};
...
versor.delta = function(v0, v1, alpha) {
  ...
  var w = cross$3(v0, v1), l = sqrt$3(dot(w, w));
  if (!l) return [1, 0, 0, 0];
  var t = alpha * acos$2(max$3(-1, min$2(1, dot(v0, v1)))) / 2, s = sin$3(t); // t = θ / 2
...
const zoom$1 = (projection, path, svg) => {
  window.svg = svg;
  ...
  const zoomed = () => {
    projection.scale((event.transform.k * (svg.clientHeight - 10)) / 10);

    var v1 = versor.cartesian(projection.rotate(r0).invert(mouse(svg))),
...
const drag$1 = (projection, path, svg) => {
  let v0; // Mouse position in Cartesian coordinates at start of drag gesture.
  let r0; // Projection rotation as Euler angles at start.
  let q0; // Projection rotation as versor at start.
  const dragstarted = () => {
  ...
  mounted() {
    ...
    this.$watch(
      ...
      val => {
        ...
        select(this.svg).call(drag$1(this.projectionFunc, this.path, this.svg));
        select(this.svg).call(zoom$1(this.projectionFunc, this.path, this.svg));
      },
...
__vue_render__._withStripped = true;
  ...
  var mapProvider = normalizeComponent_1(
...
__vue_render__$1._withStripped = true;
  ...
  var ComposableMap = normalizeComponent_1(
...
function isString(geo) { return typeof geo === "string" }
...
__vue_render__$2._withStripped = true;
  ...
  var Geographies = normalizeComponent_1(
...
__vue_render__$3._withStripped = true;
  ...
  var Geography = normalizeComponent_1(
...
var script$7 = {
  ...
  computed: {
    lineData() {
      ...
      return this.context.path ? this.context.path({
        type: "LineString",
